...
IMAGES_DATASETS = ['svhn', 'mnist', 'usps', 'clipart', 'product', 'carpet', 'wood', 'leather']
...
def get_percentile(scores, dataset):
    if dataset == 'kdd':
        ...
        # Highest 2.13% are anomalous
        per = np.percentile(scores, 97.5)
    else:
        per = np.percentile(scores, 10)
    return per
...
def save_results(scores, true_labels, model, dataset, method, weight, label,
    ...
    if num == None:
        ...
        num = int(num)
        per = np.sort(scores)[::-1][num - 1]
    y_pred = (scores >= per)

    print ("y pred sum", np.sum(y_pred))
    print ("true labels sum", np.sum(true_labels))

    #print (y_pred.astype(int))
    if dataset in IMAGES_DATASETS:
        print("Testing at step %i, method %s: AUROC = %.2f"
            % (step, method, 100*roc_auc))
        return 100*roc_auc
    else:
